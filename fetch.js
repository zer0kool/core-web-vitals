...
getData = async (origin) => {
  ...
  const request = [];
  const sum = await CrUXApiUtil.query({
    origin: `https://${origin}/`
  }, {
    formFactor: "Sum"
  });
  const phone = await CrUXApiUtil.query({
    origin: `https://${origin}/`,
    formFactor: "PHONE"
  }, {
    formFactor: "Phone"
  });
  const desktop = await CrUXApiUtil.query({
    origin: `https://${origin}/`,
    formFactor: "DESKTOP"
  }, {
    formFactor: "Desktop"
  });
  const tablet = await CrUXApiUtil.query({
    origin: `https://${origin}/`,
    formFactor: "TABLET"
  }, {
    formFactor: "Tablet"
...
function buildCard(labeledMetrics, origin) {
  ...
  document.querySelector(`#${siteName} .close`).onclick = function () {
    removeCard()
  };
